...
<%}%><%conn.Open();%><div class="gonggao marquee" id="m_<%=Module_Id%>"><a class="prev"></a><a class="next"></a>
  <div class="inner">
   <ul>
<% 
...
dr=dt.Rows[i];
%>
<li class="zx"><a href="<%=Detail_Url(dr)%>" target="<%=Target%>" title="<%=Server.HtmlEncode(dr["title"].ToString())%>"><%=SubStr(dr["title"].ToString(),Title_Num,true)%></a></li>
<%
 }
%>
   </ul>
