...
<%}%><%conn.Open();%><ul class="article_time">
<% 
...
%>
<li><div class="time"><span class="t1"><%=((DateTime)dr["thedate"]).ToString("dd")%></span><span class="t2"><%=((DateTime)dr["thedate"]).ToString("yyyy-MM")%></span></div><div class="tits"><a href="<%=Detail_Url(dr)%>"><%=dr["title"].ToString()%></a><span><%=SubStr(dr["pa_introduct"].ToString(),100,true)%></span></div></li>
<%
 }
%>
</ul><%conn.Close();%>
