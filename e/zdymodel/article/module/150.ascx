...
<%}%><%conn.Open();%><ul class="tw_articlea">
<% 
...
%>
<li class="tiyitiao">
<a href="<%=Detail_Url(dr)%>" class="tw_pics"><img src="<%=dr["titlepic"].ToString()%>"></a>
<a href="<%=Detail_Url(dr)%>" class="tw_tits"><%=SubStr(dr["title"].ToString(),Title_Num,true)%></a>
<div class="tw_jjs"><%=SubStr(dr["pa_introduct"].ToString(),90,true)%></div>
</li>
<%
 }else{
%>
