...
<%if(IsSearch==0){conn.Open();%><ul class="wenzhang">
<% 
...
%>
<li class="wz_li"><a href="<%=Detail_Url(dr)%>" class="wz_li_tit"><%=dr["title"].ToString()%></a><span class="wz_li_time"><%=((DateTime)dr["thedate"]).ToString("yyyy-MM-dd")%></span><span class="wz_li_jj"><%=SubStr(dr["pa_introduct"].ToString(),150,true)%></span></li>
<%
 }
%>
</ul><%conn.Close();}
