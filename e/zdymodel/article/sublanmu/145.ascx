...
<%if(IsSearch==0){conn.Open();%><ul class="wz_time">
<% 
...
%>
<li><div class="fl_time"><span class="time_t1"><%=((DateTime)dr["thedate"]).ToString("dd")%></span><span class="time_t2"><%=((DateTime)dr["thedate"]).ToString("yyyy-MM")%></span></div><div class="fr_com"><a href="<%=Detail_Url(dr)%>" class="com_tits"><%=dr["title"].ToString()%></a><span class="com_coms"><%=SubStr(dr["pa_introduct"].ToString(),250,true)%></span></div></li>
<%
 }
%>
</ul><%conn.Close();}
