...
%>
<li><a href="<%=Detail_Url(dr)%>"><span class="s1"><%=dr["title"].ToString()%></span><span class="s2"><%=((DateTime)dr["thedate"]).ToString("yyyy-MM-dd")%></span><span class="s3"><%=SubStr(dr["pa_introduct"].ToString(),50,true)%></span></a></li>
<%
 }
%>
</ul><%conn.Close();%>
