...
<%if(IsSearch==0){conn.Open();%><ul "class=items">
<% 
...
<div class="author left"><%=dr["username"].ToString()%></div>
<div class="fenge left">|</div>
<div class="comments left">评论数：<span><%=dr["comments"].ToString()%></span></div>
<div class="thedate right" data-date="<%=dr["thedate"].ToString()%>">1天前</div>
