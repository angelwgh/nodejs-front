...
<tr id='tr_bbs_pa_tiezicont'><td class='tdhead'>帖子内容</td><td><textarea name='pa_tiezicont' id='pa_tiezicont' style="width:100%;height:300px"   ><%if(post=="add"){Response.Write("");}else{Response.Write(r("pa_tiezicont"));}%></textarea><script type="text/javascript">var KE_pa_tiezicont;KindEditor.ready(function(K) {KE_pa_tiezicont= K.create("#pa_tiezicont",{uploadJson :kindeditor_uploadJson,fileManagerJson :kindeditor_fileManagerJson,allowImageUpload:<%=Editor_ImageUpload%>,allowFlashUpload:<%=Editor_FlashUpload%>,allowMediaUpload:<%=Editor_FlashUpload%>,allowFileUpload:<%=Editor_AttachmentUpload%>,allowFileManager :false,items :kindeditor_SmallItems,filterMode :false,extraFileUploadParams:{siteid:"<%=SiteId%>"}});});</script><span id="pa_tiezicont_tip"></span></td></tr>
