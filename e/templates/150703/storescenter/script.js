...
!function () {
	...
	function parsingData (obj) {
		...
		option.totlePageCount =this.data[option.type].length==0?1: Math.ceil(this.data[option.type].length/option.pageSize);
		option.lastPageSize  = (this.data[option.type].length)%(option.pageSize);
		option.lastPageSize  = option.lastPageSize==0?option.pageSize:option.lastPageSize;
		//console.log(option)
		return option
	}
		...
		for(var i = 0 ; i < length; i++){
			...
			var thum_imgs='<ul class="thum_imgs">';
				...
				thum_imgs.on('click','img', function(event) {
					...
					var imgUrl = $(event.target).attr('data-imgUrl');

					console.log(imgUrl)
					var click_id ='#' + $(event.target).attr('data-clickId');
	...
	function drawPage () {
		...
		if (location.hash == "") { //默认参数
			hash ={type:645,pageIndex:1,pageSize:floorView.type}
		}
